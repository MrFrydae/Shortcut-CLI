...
pub async fn run(
    ...
    let template_name = args
        .name
        .as_ref()
        .map(|n| n.parse::<api::types::UpdateEntityTemplateName>())
        .transpose()
    ...
    // Determine if any story content flags were provided
    let has_story_content = args.story_name.is_some()
        || description.is_some()
        || args.story_type.is_some()
        || !args.owner.is_empty()
        || args.state.is_some()
        || args.epic_id.is_some()
        || args.estimate.is_some()
        || !args.labels.is_empty()
        || args.iteration_id.is_some()
        || !args.custom_fields.is_empty();

    let story_contents = if has_story_content {
        ...
        let mut contents = api::types::UpdateStoryContents::default();

        if let Some(name) = &args.story_name {
            contents.name = Some(name.clone());
        }
    ...
    let template = client
        ...
        .body_map(|mut b| {
            ...
            if let Some(sc) = story_contents {
                b = b.story_contents(Some(sc));
            }
            b
        })
        .send()
        .await
        .map_err(|e| format!("Failed to update entity template: {e}"))?;
    ...
    out_println!(out, "Updated template {} - {}", template.id, template.name);
    Ok(())
}
